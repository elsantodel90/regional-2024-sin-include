...
{
    visited[node] = true;
    targetValues[node] = targetNode < 0 ? b[node] : 0;
    for (Arista *edge = g[node]; edge; edge = edge->siguiente)
    {
        int y = edge->nodo;
        if (!visited[y])
            dfs(y);
        targetValues[node] += targetValues[y];
        targetValues[node] %= MOD;
    }
    if (targetNode == node)
        targetValues[node] = 1;
}
...
int query[MAXN];
int mat[MAXN][MAXN]; // mat[i][j] es la cantidad de caminos de j hasta i

const int MAX_UPDATES = 90;

struct Update
{
    int node;
    int delta;
} updates[MAX_UPDATES+10];
...
int main()
{
    ...
    forn(index,E)
    {
        if (*readString() == 'U')
        {
            Update &u = updates[updateCount];
            u.node = int(readInt())-1;
            int newValue = int(readInt());
            u.delta = newValue - b[u.node];
            b[u.node] = newValue;
        ...
        {
            queryIndex++;
            int node = int(readInt())-1;
            if (updateCount > MAX_UPDATES)
            {
                updateCount = 0;
                callDfs(-1, query);
            }
            tint queryRet = query[node];
            forn(i, updateCount)
            {
                // #caminos de node -> y
                int y = updates[i].node;
                queryRet += tint(mat[y][node]) * updates[i].delta;
                queryRet %= MOD;
            }
            ret += (tint(queryIndex)*queryRet)%MOD;
            ret %= MOD;
        }
    }
    printIntLn((ret%MOD+MOD)%MOD);
